/*! 
 *  @file     kalman.c
 *  @brief    Implements a gyro-bias tracking kalman filter.
 *  @detailed To help others to understand the kalman filter, I used one
 *            of the most accessible sources with information on it: 
 ...
 *  @date     17-oct-2007
 *  @since    0.1
 *  @todo     Implement a check that when one of the state variables if NaN, the
 *            model is automatically reset.
 */
...
/*!
 *  Initializes the filterstate.
 *
 *  @param filterdata Filterstate to initialize.
 */
void init_Gyro1DKalman(struct Gyro1DKalman *filterdata, float Q_angle, float Q_gyro, float R_angle)
{
	...
	filterdata->x_angle = 0.0;
	filterdata->x_bias = 0.0;
}

void kalman_CopyFromTo (struct Gyro1DKalman *filterdata1, struct Gyro1DKalman *filterdata2)
{
	filterdata2->P_00 = filterdata1->P_00;
	filterdata2->P_01 = filterdata1->P_01;
	filterdata2->P_10 = filterdata1->P_10;
	filterdata2->P_11 = filterdata1->P_11;
	filterdata2->Q_angle = filterdata1->Q_angle;
	filterdata2->Q_gyro  = filterdata1->Q_gyro;
	filterdata2->R_angle = filterdata1->R_angle;
	filterdata2->x_angle = filterdata1->x_angle;
	filterdata2->x_bias = filterdata1->x_bias;
}	
...
{
	if (filterdata->x_angle != filterdata->x_angle)
	{
		filterdata->x_angle = 0;
		printf ("NaN detected!!!!\n\r");
		if (filterdata->x_bias != filterdata->x_bias)
		{
			filterdata->x_bias = 0.0;
			filterdata->P_00 = 1.0;
			filterdata->P_01 = 0.0;
 ...
 *
 */
float ars_predict(struct Gyro1DKalman *filterdata, const float dotAngle, const float dt)
{
 ...
 *
 *  @param angle_m The measured angle as input into the filter.
...
/*!
 *  Combination of the predict and update function.
 *
 */
